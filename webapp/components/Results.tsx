...
const Results = async ({ query }: any) => {
        ...
        <div className="bg-gray-100 p-6 shadow-xl rounded-lg max-w-[800px] mx-auto mt-8">
          <div className="mb-5">
            {sentenceData.map((sentence) => (
              <div key={sentence.id}>
                <h2 className="mb-5 text-3xl font-bold tracking-tight text-gray-900 dark:text-white">
                  {sentence.sentence}
                </h2>
                <video
                  key={sentence.id}
  ...
  let entryData = await db
    .select()
    .from(entry)
    .limit(1)
    .where(eq(entry.entryInEnglish, query));
  ...
  if (entryData.length === 0) {
    ...
    for (let res of results) {
      if (!uniqueWordsIndex.has(res.metadata.row)) {
        // Check if word is unique based on row
        ...
        const match = await db.query.worddefinitions.findFirst({
          ...
          where: (worddefinitions, { eq }) =>
            eq(worddefinitions.id, res.metadata.row),
        });
        ...
        if (uniqueWords.size === 3) {
          // Check if we have enough unique words
          break;
        }
      }
    }
  ...
  let subEntries = await db
    .select()
    .from(subentry)
    .where(eq(subentry.entryId, entryData[0].id));
