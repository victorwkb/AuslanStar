  ...
  const handleShowGuide = () => {
   ...
   useEffect(() => {
    ...
    const initialize = () => {
        ...
        switch (difficulty) {
            case 'easy':
                pairsToUse = shuffle(boardPairs).slice(0, 6);
                break;
            case 'medium':
                pairsToUse = shuffle(boardPairs).slice(0, 9);
                break;
            case 'hard':
                pairsToUse = shuffle(boardPairs).slice(0, 18);
                break;
            default:
                pairsToUse = shuffle(boardPairs).slice(0, 18); // Defaults to 'hard' if somehow an unknown difficulty is selected
        }
